...
We will be using many of the same packages you have used in previous labs. However, you'll need to install a few MIP packages using the `install.packages()` function.
```{r}
...
In other words, the summation of the number of items taken * their value, such that the weight of all the items is less than the maximum weight.
\
\
...
There are many languages and packages that can be used to implement Mixed Integer Programs. Today, we will use a slower, but more user-friendly package called `ompr`.
...
If you have indexed variables, then you often want to sum over a subset of variables.The following code creates a model with three decision variables $x_1, x_2, x_3$, an objective function, *maximize* $\sum_ix$, and one constraint $\sum_{i=1}^{3}x_i<=10$ 
...
cbind(weights,benefits) #print weights an values
...
This code will show you which items were chosen (i.e. which decision variables set to a value of 1 by the model)
```{r}
...
Let's reimagine the knapsack problm as a 3x3 grid of planning units. Each of these can be thought of as a separate 'knapsack' (though we'll talk later about how dispersal makes this assumption invalid). 

Let's first imagine we have a fixed budget of \$50. While we will be focusing on a fixed budget approach here, note that there are also alternative approaches, like maximizing the ratio of benefits to costs, or maximizing the difference between benefits and costs, and these can be implemented just as easily in MIP. We can choose to apply an insecticide treatment to each of the cells that costs \$10, or we can perform an aerial spray on the cell that costs $20. The first of these interventions causes 50% reduction in the pest population at each cell, whereas the second causes a 90% reduction in each cell. 
...
Let's also assume that we previous collected data on (or had modelled) the relative abundance of the pest across sites.
...
**(8)** In the code chunk below, rerun this model with a budget of 100, reducing the efficiency of insecticide to 25%. (HINT: you can copy and paste the entire model code as is, the only thing you need to change are the constants you set earlier, `B` and `e_t`)
...
At this stage, the model specification is already quite complex, but we haven't accounted for much of what makes invasion management problems difficult! Namely, we need to look at the role of temporal and spatial non-independence. That is, management at one time and place has often-unpredictable feedbacks on every other location across time.

In invasions, this is not just due to population recovery through growth, but also because of dispersal. Both of these processes make the specification of linear problems quite difficult (as you saw in lecture)
...
##### **Questions**
**(8)** What is still lacking from the realism of this model?
\
\
...
**(12)** Describe which parameters you would change to improve the objective function further. If you are feeling brave, try changing these and resolving the model in the code chunk below!
